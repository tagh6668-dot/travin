            ...
            <?php if($session->goldclub){ ?>
                ...
                if($fList['id']){ 
                    if(($uinfo['ptime'] > time() || $uinfo['protect']==1)){
                        echo "<span class=\"a arrow disabled\" title=\"player Under beginner protectors \">اضافه کردن به فارم لیست</span>";
                    }else{
                    ?>
                    <a href="build.php?id=39&t=99&action=showSlot&lid=1&vid=<?php echo $getd; ?>&sort=distance&direction=asc"><span class="a arrow GoldClub">اضافه کردن به فارم لیست</span></a>
                <?php } }else{ ?>
                    <span class="a arrow disabled" title="Create a list of farms first From a collection point">اضافه کردن به فارم لیست</span></span>
