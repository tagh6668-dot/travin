...
$file = APP_MAIN_PATH . "/artefacts.txt";

if (ARTEFACTS <= time() && !file_exists($file)) {
...
$file = APP_MAIN_PATH . "/ww_plans.txt";

if (WW_PLAN <= time() && !file_exists($file)) {

    // تلاش برای ساخت فایل و بررسی نتیجه
    if (file_put_contents($file, "1") !== false) {

        // فقط وقتی فایل با موفقیت ایجاد شد، ادامه بده
        $this->showWWPlans();
