...
if(isset($_POST['access']) || isset($_GET['access'])){
    unset($_POST['access']);
    unset($_GET['access']);
}
...
class Session {
  ...
  public function Login($user) {
    ...
    // کوئری امن
    $q = $database->queryFetch(
        'SELECT username FROM users WHERE username = :user1 OR email = :user2',
        array(
            'user1' => $user,
            'user2' => $user
        )
    ...
    $this->sit = isset($_SESSION['sit']) ? $_SESSION['sit'] : null;
   ...
   private function checkLogin(){
    ...
    if(isset($_SESSION['username']) && isset($_SESSION['sessid'])) {
        ...
        // جلوگیری از مقادیر غیرمنتظره
        if(empty($username) || empty($sessid)){
            $this->Logout();
            return false;
        }

        // بررسی آنلاین بودن سشن
        $mas = count($database->GetAOnline2($sessid));
        ...
        // جلوگیری از خطای timestamp
        if(!isset($_SESSION['timestamp'])){
            $_SESSION['timestamp'] = time();
        }

        // آپدیت timestamp در دیتابیس
        if(time() - $_SESSION['timestamp'] > 30){
            $_SESSION['timestamp'] = time();
            $database->updateUserField($username, "timestamp", time(), 0);
        }
...
private function PopulateVar() {
    ...
    if(!$u){
        $dData = $database->queryFetch(
            "SELECT * FROM deleted WHERE username = :user",
    ...
    if(isset($_GET['getIn']) && $u['access'] == 9){
        $target = preg_replace('/[^a-zA-Z0-9_\-]/','',$_GET['getIn']);

        $check = $database->queryFetch(
            "SELECT id FROM users WHERE username = :user",
            array('user'=>$target)
        );

        if(!empty($check['id']) && $target != $_SESSION['username']){
    ...
    if($u['msg'] == 0 && $this->page != 'messages.php' && $u){
        if(isset($_GET['ok'])){
            $database->query(
                "UPDATE users SET msg = 1 WHERE id = :id",
                array('id'=>$u['id'])
            );
            header('Location: dorf1.php');
            exit;
    ...
    if(!$u['vid']){
        $database->query(
            "UPDATE users SET vid = :vid WHERE id = :id",
            array(
                'vid'=>$vilmas[0][0],
                'id'=>$u['id']
            )
        ...
        $vCheck = $database->queryFetch(
            "SELECT owner FROM vdata WHERE wref = :wref",
            array('wref'=>$u['vid'])
        );
    ...
    $qData = $database->queryFetch(
        "SELECT * FROM quests WHERE userid = :uid",
        array('uid'=>$u['id'])
    );

    $this->fData = $database->queryFetch(
        "SELECT * FROM fdata WHERE vref = :vref",
        array('vref'=>$_SESSION['wid'])
    );

    $this->acData = $database->queryFetch(
        "SELECT * FROM achiev WHERE uid = :uid",
        array('uid'=>$u['id'])
    );
    ...
    $this->link = isset($_SESSION['dorf']) ? $_SESSION['dorf'] : 1;
    ...
    $this->checker = isset($_SESSION['checker']) ? $_SESSION['checker'] : null;
    $this->mchecker = isset($_SESSION['mchecker']) ? $_SESSION['mchecker'] : null;
    ...
    $this->bonus1 = (time() < $u['b1']) ? $u['b1'] : 0;
    $this->bonus2 = (time() < $u['b2']) ? $u['b2'] : 0;
    $this->bonus3 = (time() < $u['b3']) ? $u['b3'] : 0;
    $this->bonus4 = (time() < $u['b4']) ? $u['b4'] : 0;

    $this->goldclub  = $u['goldclub'];
    $this->silver = $_SESSION['silver'] = $u['silver'];
    $this->plust = $u['plus'];
    $this->sit = isset($_SESSION['sit']) ? $_SESSION['sit'] : 0;
    ...
    if($u['advtime'] + ADV_TIME < time()){
        ...
        if($advs>0){
            $database->newAdvTime(
                $this->uid,
                time()-(time()-$u['advtime']-($advs*ADV_TIME))
            );
        }
    }
